...
struct PokedexProjectTests {
    ...
    func fetch_EntryUseCaseWhenStatusCode200() async throws -> Void {
    ...
    func fetch_EntryUseCaseWhenStatusCode400() async throws {
        ...
        do {
            //  When
            let _ = try await sut?.request(with: endpoint).results
        } catch {
            //  Then
            print("***** 테스트 실패 *****")
